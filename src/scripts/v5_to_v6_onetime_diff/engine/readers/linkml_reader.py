...
from ..models import NormalizedSchema, NormalizedTerm
from .base import BaseReader, detect_format
...
class LinkMLReader(BaseReader):
    ...
    def read(self, path: str, version: Optional[str] = None) -> NormalizedSchema:
        ...
        # Note: we do not call materialize_patterns() here, on purpose. Patterns
        # are not part of this comparison: the v5 Excel model asserts no patterns
        # (only a free-text "Value syntax" column, mapped to value_syntax), and
        # MIxS v6.0.0 has zero structured_patterns (they were added after v6.0.0).
        # So there is nothing to materialize on either side. The reusable
        # diff-releases tool does materialize, which is correct for modern MIxS.
    ...
    def _extract_terms(self, schema_view: SchemaView, schema: NormalizedSchema) -> None:
        ...
        Reads the *induced* slot for each name (induction on), so is_a parents,
        mixins, and slot-level overrides are resolved rather than reading the raw
        top-level declaration. This treats the input as a LinkML schema, not as a
        flat YAML table. Per-class slot_usage still varies by class and is handled
        for membership in _extract_checklists_and_packages.
        """
        for slot_name in schema_view.all_slots(imports=True):
            slot_def = schema_view.induced_slot(slot_name, imports=True)
            term = NormalizedTerm(
                name=slot_name,
            ...
            # Store in annotations for additional metadata. Induced slots return
            # annotations as a JsonObj (no .items()), so use jsonasobj2.items,
            # which handles dict and JsonObj alike.
            if slot_def.annotations:
                from jsonasobj2 import items as _jsonobj_items
                for ann_name, ann_val in _jsonobj_items(slot_def.annotations):
