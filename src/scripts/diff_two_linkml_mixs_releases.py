        ...
        if hasattr(value, '_as_dict'):
            try:
                ...
                if hasattr(value, '__class__') and value.__class__.__name__.endswith('Definition'):
                    ...
                    cleaned_dict = {}
                    # slot_uri and class_uri are compared because an identifier
                    # change is breaking for consumers, and was previously the one
                    # kind of change a release diff could not report.
                    important_fields = ['name', 'description', 'title', 'meaning', 'pattern', 'range', 'required', 'multivalued', 'slot_uri', 'class_uri']
...
def _ref(spec: str) -> str:
    """Reduce an owner/repo@ref:path spec to a filesystem- and label-safe ref.

    Requires both separators, so a ref cannot nest directories or escape
...
def build_tool_summary(comparison_dict: Dict[str, Any], old: str, new: str) -> str:
    """Render the counts in a comparison result as a short Markdown summary.

    Derived entirely from the structured output, so it can be regenerated and
    checked. The readable prose summary is a separate, human or agent authored
    file (agent_summary.md).
    """
    def n(x):
        return len(x) if hasattr(x, '__len__') else (x or 0)

    old_label, new_label = _ref(old), _ref(new)
    lines = [f"# MIxS schema diff: {old_label} to {new_label}", ""]
    scalars = comparison_dict.get('scalar_differences') or {}
    if scalars:
    ...
    for kind, block in sorted((comparison_dict.get('collection_differences') or {}).items()):
        ...
        lines.append(f"## {kind}")
        lines.append("")
        only_old = n(key_cmp.get('only_in_old'))
        only_new = n(key_cmp.get('only_in_new'))
        shared = n(key_cmp.get('shared'))
        lines.append(f"- only in {old_label}: {only_old}")
        lines.append(f"- only in {new_label}: {only_new}")
        if shared:
            lines.append(f"- in both: {shared}")
        lines.append(f"- definition changes: {n(block.get('definition_changes'))}")
        lines.append("")

    lines += ["Generated by `diff-releases` from the structured comparison in this",
