...
# DANGER: this deletes the diff outputs under assets/diff_results/ (the structured
# diffs and their summaries). They can be regenerated by re-running the diff
# scripts, but they are also committed, so if you delete them and then commit and
# push, you remove them from the repository. Do not run this if you intend to push
# your current work. It refuses to run unless you pass CONFIRM=1.
CONFIRM ?=
.PHONY: clean-diff-results
clean-diff-results:
	@echo "DANGER: this removes the regeneratable diff outputs under assets/diff_results/"
	@echo "        (structured diffs and their .md summaries). These are committed, so if"
	@echo "        you commit and push afterwards, those files leave the repository."
	@echo "        Re-run with 'make clean-diff-results CONFIRM=1' to proceed."
	@test "$(CONFIRM)" = "1" || exit 1
	find assets/diff_results -type f \( -name '*.yaml' -o -iname '*summary*.md' \) -delete
	@echo "Removed regeneratable diff outputs. Re-run the diff scripts to regenerate."
